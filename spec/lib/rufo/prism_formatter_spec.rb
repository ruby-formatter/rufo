...
def assert_format(code, expected = code, **options)
  ...
  opts = options.merge(engine: :prism)
...
RSpec.describe Rufo::PrismFormatter do
  ...
  #   Dir[File.join(FILE_PATH, "/prism_formatter_source_specs/3.1/*")].each do |source_specs|
  #     assert_source_specs(source_specs) if File.file?(source_specs)
  #   end
  # end

  # if VERSION >= Gem::Version.new("3.2")
  #   Dir[File.join(FILE_PATH, "/prism_formatter_source_specs/3.2/*")].each do |source_specs|
  #     assert_source_specs(source_specs) if File.file?(source_specs)
  #   end
  # end
  ...
  describe "Syntax errors handled by Prism" do
    it "raises an syntax error" do
      expect {
        Rufo.format("def foo; FOO = 1; end", engine: :prism)
      }.to raise_error(Rufo::SyntaxError)
    end
  end
end
